...
public class WishlistController {
    ...
    @PostMapping("/create")
    public ResponseEntity<Wishlist> createWishlist(@RequestHeader("Authorization") String jwt) throws UserException {
    ...
    public ResponseEntity<Wishlist> deleteProductFromWishlist(
            @PathVariable Long productId,
            @RequestHeader("Authorization") String jwt) throws WishlistNotFoundException, ProductException, UserException {

        Product product = productService.findProductById(productId);
        User user = userService.findUserProfileByJwt(jwt);
        Wishlist updatedWishlist = wishlistService.deleteProductFromWishlist(user, product);
